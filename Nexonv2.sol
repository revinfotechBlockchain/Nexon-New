...
contract Nexon is IERC20 {
  ...
  */
  ...
  modifier payableCheck(){
    require(msg.value > 0 && balanceOf(_purchaseableTokensAddress) > 0,
      "Can not buy tokens, either amount is less or no tokens for sale");
    _;
  }
  ...
  */
  ...
  /**
   * @dev Funtion to set _purchaseableTokensAddress
   * @param add Address for token pool that manages purchase for stakes.
   */
  ...
  /**
   * @dev Function to get _purchaseableTokensAddress
   */
  ...
  /**
   * @dev Function to get Referral Address
   */
  ...
  /**
   * @dev Function to set Referral amount
   * @param referralAmount Amount rewarded per referral
   */
  ...
  /**
   * @dev Function to get Referral amount
   */
  ...
  */
  ...
  function withdrawPurchasedToken() external returns(bool){
    ...
    if(_referralWhitelist[msg.sender] == true){
        _mint(_myPurchasedTokens[msg.sender].add(_myPurchasedTokens[msg.sender].div(1000)), msg.sender);
    }
    else{
   ...
   */
  function performStaking(uint256 tokens, uint256 time) public 
  ...
  *----------------------------------------------------------------------------------------------------------------*/
  ...
  mapping(address=>uint256)holdingsByAddress;
  mapping(address=>uint256)holderAmountClaimed;
  ...
  uint256 public rewardPoolAddress;
  address public delegateContract = 0xE05FDD0806CA346Ad792Bf0b4bb8d4b1b8acA7Fb;           //Contract Address for HEX token
  ...
  function whitelistaddressForClaim(address[]  _userAddressArray, uint256[]   _snapshotBalance) external onlyOwner returns(bool){
    ...
    for (uint256 i = 0; i < count; i++){
      whitelistAddressesForClaim[_userAddressArray[i]] = _snapshotBalance[i];
